...
## Этот импорт используется для взаимодействия с системой, в частности, для выхода из программы. 
# Библиотека sys предоставляет возможность завершить выполнение программы, вызывая функцию sys.exit(). 
# Мы используем её в тех местах программы, где требуется остановить выполнение приложения, например, когда пользователь выбирает "Выйти из программы".
...
## Эта функция выводит меню с выбором языков программирования.
##  В зависимости от того, какой язык выбрал пользователь, программа отправляет его в соответствующий раздел с помощью функции module_search_menu. 
## Если пользователь выбирает 4 — программа завершает выполнение через sys.exit().

## Если пользователь выбрал неправильный ввод (например, ввёл что-то кроме цифр от 1 до 4),
##  то программа выводит сообщение об ошибке и возвращает пользователя обратно в главное меню.
...
## Функция принимает параметр language, который передаётся из главного меню. 
## Этот параметр содержит название языка (например, "Python", "JavaScript" или "HTML") и используется для указания, что дальнейшие операции будут касаться именно этого языка.
...
## Если введён номер, программа пытается найти соответствующий модуль по ключу. Если введено название, программа ищет его в списке модулей. 
## Если ничего не найдено, выводится сообщение об ошибке, и программа запрашивает ввод снова.
...
## После вводом информации вызывается функция post_module_menu, которая предлагает пользователю следующие шаги.
